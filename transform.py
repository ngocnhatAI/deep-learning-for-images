...
def gender_transforms(IMG_SIZE = 224, is_train=False):
    if is_train:
        return transforms.Compose([
            # Apply color adjustments (brightness, contrast, saturation, hue)
            ...
            # Crop to square to maintain aspect ratio before resizing
            transforms.CenterCrop((178, 178)),  # Crop to square (178x178) from input (218x178)
            # Resize image to target output size
            ...
            transforms.RandomPerspective(
                ...
                p=0.3,                       # Apply with 50% probability
                interpolation=transforms.InterpolationMode.BILINEAR,
                fill=0                       # Fill empty areas with black
            ),
